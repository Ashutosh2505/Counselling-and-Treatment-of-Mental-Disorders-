...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    if (storedUserInfo && storedUserCreds) {
        ...
        const userEmail = storedUserCreds.email;
        ...
        const phoneRef = ref(db, `UserInformation/${userId}/phone`);
            get(phoneRef).then((snapshot) => {
                ...
                if (snapshot.exists()) {
                    ...
                    const name = storedUserInfo.username;
                    const userEmail = storedUserCreds.email; // Accessing 'when' property
function mailit(){
    const bodyMessage = `Full Name: ${name}<br> Email: ${userEmail}<br> Phone:${phone}<br> message: User wants to book an appointment with you.`;
    Email.send({
        Host : "smtp.elasticemail.com",
        Username : "mindpathmentalhealth@gmail.com",
        Password : "09CAB801CB59905EB37A10907EE96BCA2BE2",
        To : 'azhruddin986@gmail.com',
        ...
        message => {
          if (message == "OK"){
              Swal.fire({
                  title: "Thank You!",
                  text: "Dr. Azahruddin Mohammad Hassan have recieved your request.",
                  icon: "success"
                });
          }
        }
    ...
    Email.send({
        ...
        message => {
          if (message == "OK"){
              Swal.fire({
                  title: "Thank You!",
                  text: "Dr. Avinash Saini have recieved your request.",
                  icon: "success"
                });
          }
        }
                    ...
                    document.getElementById("bookAppointment1").addEventListener("click", mailit);
                    document.getElementById("bookAppointment2").addEventListener("click", mailit1);
                }
                })
