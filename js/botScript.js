...
document.getElementById('name').textContent = `${storedUserInfo.username}`;
        ...
        function addProblem() {
          ...
          const problemInput = document.getElementById("problem" + problemCount);
          const subproblemInput1 = document.getElementById("subproblem1");
          const subproblemInput2 = document.getElementById("subproblem2");
          const problem = problemInput.value.trim();
          const subproblem1 = subproblemInput1.value.trim();
          const subproblem2 = subproblemInput2.value.trim();
          ...
          if (problem !== "") {
            ...
            set(ref(db,`selfCounseling/${userId}/problem`),{
                 problem:problem,
            ...
            problemInput.value = "";
            subproblemInput1.value = "";
            subproblemInput2.value = ""; // Clear input field
            problemCount++;
          }
        }
        function addWhen() {
            ...
            const whenInput = document.getElementById("sincewhen");
        
            const when = whenInput.value.trim();
        
            const step1 = document.getElementById("step2");
            const step2 = document.getElementById("step3");
            if (when !== "") {

                set(ref(db,`selfCounseling/${userId}/when`),{
                    when:when
                })
                
                  .then(() => {
                    console.log("when added successfully!");
                    document.getElementById("step2").style.display = "none";
                    document.getElementById("step3").style.display = "block";
                    const cwhenRef = ref(db, `selfCounseling/${userId}/when`);
            get(cwhenRef).then((snapshot) => {
                console.log("Snapshot exists:", snapshot.exists());
                if (snapshot.exists()) {
                    const cwhen = snapshot.val().when; // Accessing 'when' property
                    console.log("cwhen:", cwhen);
                    document.getElementById('cwhen').textContent = "your mentioned incidence: "+cwhen; }
                })
                const cproblemRef = ref(db, `selfCounseling/${userId}/problem`);
            get(cproblemRef).then((snapshot) => {
                console.log("Snapshot exists:", snapshot.exists());
                if (snapshot.exists()) {
                    const cproblem = snapshot.val().problem; // Accessing 'when' property
                    console.log("cproblem:", cproblem);
                    document.getElementById('cproblem').textContent = "your mentioned issue: "+cproblem; }
                })
                whenInput.value="";
            })
        ...
        function adddafu(event) {
            ...
            const dailyworkChecked = document.querySelector('input[name="dailywork"]:checked');
            const dietChecked = document.querySelector('input[name="diet"]:checked');
            const substanceChecked = document.querySelector('input[name="substance"]:checked');
            
            const weight = weightChecked ? true : false;
            const dailywork = dailyworkChecked ? true : false;
            const diet = dietChecked ? true : false;
            const substance = substanceChecked ? true : false;

            const step3 = document.getElementById("step3");
            const step4 = document.getElementById("step4");

                const dafuRef = ref(db,`selfCounseling/${userId}/dafu`);
                set(ref(db,`selfCounseling/${userId}/dafu`),{
                     weight: weight,
                     dailywork:dailywork,
                     diet:diet,
                     substance:substance
                  ...
                  .then(() => {
                    console.log("dafu added successfully!");
        ...
        function addcop() {
            ...
            const cop = copInput.value.trim();
        
            const step1 = document.getElementById("step2");
            const step2 = document.getElementById("step3");
            if (cop !== "") {

                set(ref(db,`selfCounseling/${userId}/cop`),{
                    cop:cop
                })
                
                  .then(() => {
                    console.log("cop added successfully!");
        ...
        function addimpr() {
            ...
            const imprInput = document.getElementById("impr");
        
            const impr = imprInput.value.trim();
        
            const step1 = document.getElementById("step2");
            const step2 = document.getElementById("step3");
            if (impr !== "") {

                set(ref(db,`selfCounseling/${userId}/impr`),{
                    impr:impr
                })
        ...
        document.getElementById("addWhenBtn").addEventListener("click", addWhen);
        document.getElementById("adddafuBtn").addEventListener("click", adddafu);
        document.getElementById("addcopBtn").addEventListener("click", addcop);
        document.getElementById("addimprBtn").addEventListener("click", addimpr);
